...
export async function createServer(input?: Partial<AppConfig>): Promise<Server> {
  ...
  const ready = (async () => {
    ...
    registerReferenceResources(registerResource, context);
    try {
      await context.notifier.notify("resources/list_changed", {
        resources: context.resourceList,
        resourceTemplates: context.resourceTemplateList
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
